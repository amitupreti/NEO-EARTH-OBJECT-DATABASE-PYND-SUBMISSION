    ...
    def build_query(self):
        ...
        if (self.date):
            ...
            date_search = Query.DateSearch(
                DateSearch.between, [self.start_date, self.end_date])

        query = Query.Selectors(
            date_search, self.number, self.filters, Query.ReturnObjects[self.return_object])
            ...
            for filter_type in filter_options:
                ...
                if x[0] in ["diameter", Filter.Options["is_hazardous"]]:
                    defaultdict["NEO"].append(
                        Filter(x[0], NearEarthObject, x[1], x[2]))

                if x[0] == Filter.Options["distance"]:
                    defaultdict["ORB"].append(
                        Filter(x[0], OrbitPath, x[1], x[2]))
...
class NEOSearcher(object):
    ...
    def between_search(self, date_search):
        ...
        daterange = [
            start_date + datetime.timedelta(days=x) for x in range(0, (end_date-start_date).days)]
    ...
    def get_objects(self, query):
        ...
        filters = Filter.create_filter_options(
            query.filters) if query.filters is not None else None
